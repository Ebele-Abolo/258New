...
left_frame = Frame(root, bd = 15, bg = 'white', padx = 5, pady = 6)
labelu = Label(left_frame, txt = "Enter username", bg = 'black')
labelp = Label(left_frame, txt = "Enter password", bg = 'blue')
grid(labelp = row=1, column=0, sticky = w, pady ='5')
user_id = Entry(left_frame)
password = Entry(left_frame, show('*'))

user_id.grid(row=0, column = 1, pady = 10, pady =20)
...
if user_name == 'mycryptoaccount' and user_pass '1234':
    new_WIndow = Toplevel(root)
    title(newWindow)
    newWindow.geometry(width = 15, height = 15)
    ...
    newWindow = Toplevel(root)
        ...
        def sendETH():
            ...
            tx = {
                ...
                login_btn.Button(left_frame, wdith = 14,"Login", 'hand2', cmd = openNewWindow)
                login_btn.grid(root, row = 2, column = 1, pady = 5, padx = 8)
            }
